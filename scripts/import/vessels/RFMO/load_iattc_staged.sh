#!/bin/bash
# Staged Intelligence Import for IATTC
...
log_step "🎯 IATTC Staged Intelligence Import (Phase 1: Raw Collection)"
...
if [[ ! -f "$INPUT_FILE" ]]; then
    log_error "Cleaned IATTC data not found: $INPUT_FILE"
    exit 1
fi
...
# Get IATTC source ID
IATTC_SOURCE_ID=$(execute_sql "
    SELECT source_id FROM original_sources_vessels 
    WHERE source_shortname = 'IATTC' 
    LIMIT 1;
" "-t" | head -n1 | xargs)

if [[ -z "$IATTC_SOURCE_ID" ]]; then
    log_error "IATTC source not found"
    exit 1
fi

log_success "IATTC source ID: $IATTC_SOURCE_ID"
...
execute_sql "
DROP TABLE IF EXISTS iattc_raw_staging;
CREATE TABLE iattc_raw_staging (
    source_date TEXT,
...
log_step "Loading IATTC raw data..."

# Load CSV data
execute_sql "\\copy iattc_raw_staging FROM '$INPUT_FILE' WITH CSV HEADER"

RAW_COUNT=$(execute_sql "SELECT COUNT(*) FROM iattc_raw_staging;" "-t" | xargs)
log_success "Loaded $RAW_COUNT raw IATTC records"
...
SELECT 
    ...
    NULLIF(TRIM(ir.raw_vessel_data->>'iattc_vessel_number'), ''),
    'FV',  -- IATTC doesn't provide vessel type in data
    ...
    END,
    NULLIF(TRIM(ir.raw_vessel_data->>'company_name'), ''),
    NULLIF(TRIM(ir.raw_vessel_data->>'company_address'), ''),
    COALESCE(
        NULLIF(TRIM(ir.raw_vessel_data->>'register_status'), ''),
        'ACTIVE'  -- IATTC vessels assumed active if in register
    ),
    jsonb_build_object(
        'national_registry', ir.raw_vessel_data->>'national_registry',
        'shipyard', ir.raw_vessel_data->>'shipyard',
        'previous_name', ir.raw_vessel_data->>'previous_name',
        ...
        'fish_hold_volume', ir.raw_vessel_data->>'fish_hold_volume',
        'fish_hold_volume_unit_enum', ir.raw_vessel_data->>'fish_hold_volume_unit_enum',
        'carrying_capacity', ir.raw_vessel_data->>'carrying_capacity',
        'carrying_capacity_unit_enum', ir.raw_vessel_data->>'carrying_capacity_unit_enum',
        'notes', ir.raw_vessel_data->>'notes',
        'image_available', ir.raw_vessel_data->>'image_available',
        'confirmation_date', ir.raw_vessel_data->>'confirmation_date',
        'last_modification', ir.raw_vessel_data->>'last_modification'
    ),
    ...
    (
        ...
        CASE WHEN TRIM(ir.raw_vessel_data->>'iattc_vessel_number') IS NOT NULL AND TRIM(ir.raw_vessel_data->>'iattc_vessel_number') != '' THEN 1 ELSE 0 END +
        CASE WHEN TRIM(ir.raw_vessel_data->>'company_name') IS NOT NULL AND TRIM(ir.raw_vessel_data->>'company_name') != '' THEN 1 ELSE 0 END
    )::decimal / 6.0,
...
log_step "📊 IATTC Intelligence Summary & Validation"
...
if [[ $INTELLIGENCE_COUNT -eq $INPUT_COUNT ]]; then
    log_success "✅ IATTC Phase 1 Complete: Raw Intelligence Collection"
    ...
    log_error "❌ IATTC import validation FAILED"

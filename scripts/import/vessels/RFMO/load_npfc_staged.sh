#!/bin/bash
# Staged Intelligence Import for NPFC
...
log_step "🎯 NPFC Staged Intelligence Import (Phase 1: Raw Collection)"
...
INPUT_FILE="$CLEANED_DATA_DIR/npfc_vessels_cleaned.csv"

# Verify cleaned file exists
if [[ ! -f "$INPUT_FILE" ]]; then
    log_error "Cleaned NPFC data not found: $INPUT_FILE"
    exit 1
fi
...
log_success "NPFC source ID: $NPFC_SOURCE_ID"
...
execute_sql "
DROP TABLE IF EXISTS npfc_raw_staging;
CREATE TABLE npfc_raw_staging (
    source_date TEXT,
...
log_step "Loading NPFC raw data..."

# Load CSV data
execute_sql "\\copy npfc_raw_staging FROM '$INPUT_FILE' WITH CSV HEADER"

RAW_COUNT=$(execute_sql "SELECT COUNT(*) FROM npfc_raw_staging;" "-t" | xargs)
log_success "Loaded $RAW_COUNT raw NPFC records"
...
SELECT 
    ...
    NULLIF(TRIM(ir.raw_vessel_data->>'npfc_vessel_id'), ''),
    NULLIF(TRIM(ir.raw_vessel_data->>'vessel_type_code'), ''),
    NULLIF(TRIM(ir.raw_vessel_data->>'fishing_method_code'), ''),
    CASE 
    ...
    'ACTIVE',  -- NPFC vessels assumed active if in register
    jsonb_build_object(
        ...
        'previous_flag_alpha3', ir.raw_vessel_data->>'previous_flag_alpha3',
        'previous_registry', ir.raw_vessel_data->>'previous_registry',
        'previous_port_registry', ir.raw_vessel_data->>'previous_port_registry',
        'communication_details', ir.raw_vessel_data->>'communication_details',
        'freezer_type_enum', ir.raw_vessel_data->>'freezer_type_enum',
        'freezer_unit_enum', ir.raw_vessel_data->>'freezer_unit_enum',
        'freezer_type_raw', ir.raw_vessel_data->>'freezer_type_raw',
        'freezer_unit_raw', ir.raw_vessel_data->>'freezer_unit_raw',
        'crew_size', ir.raw_vessel_data->>'crew_size',
        ...
        'beam_metric_type', ir.raw_vessel_data->>'beam_metric_type',
        ...
        'freezer_capacity', ir.raw_vessel_data->>'freezer_capacity'
    ),
    ...
    (
        ...
        CASE WHEN TRIM(ir.raw_vessel_data->>'npfc_vessel_id') IS NOT NULL AND TRIM(ir.raw_vessel_data->>'npfc_vessel_id') != '' THEN 1 ELSE 0 END +
        CASE WHEN TRIM(ir.raw_vessel_data->>'tonnage_value') IS NOT NULL AND TRIM(ir.raw_vessel_data->>'tonnage_value') != '' THEN 1 ELSE 0 END
    )::decimal / 6.0,
...
log_step "📊 NPFC Intelligence Summary & Validation"
...
if [[ $INTELLIGENCE_COUNT -eq $INPUT_COUNT ]]; then
    log_success "✅ NPFC Phase 1 Complete: Raw Intelligence Collection"
    ...
    log_error "❌ NPFC import validation FAILED"
